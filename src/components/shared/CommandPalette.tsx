  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-[15vh]" onClick={() => setOpen(false)}>
      ...
      <div className="relative w-full max-w-lg" onClick={e => e.stopPropagation()}>
        <Command className="bg-card rounded-xl shadow-2xl border border-border overflow-hidden" onKeyDown={(e: React.KeyboardEvent) => {
          ...
          <div className="flex items-center border-b border-border px-4">
            ...
            <Command.Input
              ...
              className="w-full py-3 px-3 text-sm bg-transparent border-none outline-none placeholder:text-muted-foreground"
              autoFocus
            />
            <kbd className="text-[9px] text-muted-foreground bg-muted px-1.5 py-0.5 rounded">ESC</kbd>
          </div>
          <Command.List className="max-h-80 overflow-y-auto p-2">
            ...
            {search.length === 0 && (
              <>
                <Command.Group heading="Navegación" className="text-[10px] font-semibold text-muted-foreground px-2 py-1.5 uppercase tracking-wider">
                  {views.map(v => (
                    <Command.Item key={v.id} value={`view:${v.id}`} onSelect={handleSelect}
                      className="flex items-center gap-3 px-3 py-2 rounded-lg text-sm cursor-pointer aria-selected:bg-accent aria-selected:text-accent-foreground">
                ...
                <Command.Group heading="Acciones rápidas" className="text-[10px] font-semibold text-muted-foreground px-2 py-1.5 uppercase tracking-wider">
                  <Command.Item value="nuevo:presupuesto" onSelect={handleSelect}
                    className="flex items-center gap-3 px-3 py-2 rounded-lg text-sm cursor-pointer aria-selected:bg-accent aria-selected:text-accent-foreground">
                    <Plus className="w-4 h-4 text-muted-foreground" /> Nuevo Presupuesto
                  </Command.Item>
                  <Command.Item value="nuevo:cliente" onSelect={handleSelect}
                    className="flex items-center gap-3 px-3 py-2 rounded-lg text-sm cursor-pointer aria-selected:bg-accent aria-selected:text-accent-foreground">
                    <Users className="w-4 h-4 text-muted-foreground" /> Nuevo Cliente
                  </Command.Item>
                  <Command.Item value="nuevo:transaccion" onSelect={handleSelect}
                    className="flex items-center gap-3 px-3 py-2 rounded-lg text-sm cursor-pointer aria-selected:bg-accent aria-selected:text-accent-foreground">
                    <DollarSign className="w-4 h-4 text-muted-foreground" /> Nueva Transacción
                  </Command.Item>
                </Command.Group>
              </>
            )}
            ...
            <Command.Group heading="Proyectos" className="text-[10px] font-semibold text-muted-foreground px-2 py-1.5 uppercase tracking-wider">
              {presupuestos.filter(p => p.proyecto.toLowerCase().includes(search.toLowerCase())).slice(0, 8).map(p => (
                <Command.Item key={p.id} value={p.proyecto} onSelect={() => { setOpen(false); setView('seguimiento'); }}
                  ...
                  <span className="text-[10px] text-muted-foreground">Q{(p.total / 1000).toFixed(0)}K</span>
                  <span className={`text-[9px] px-1.5 py-0.5 rounded-full font-semibold ${
                    p.fase === 'ejecución' ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300' :
                    p.fase === 'planeación' ? 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-300' :
                    p.fase === 'finalizado' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300' :
            ...
            {filteredClientes.length > 0 && (
              <Command.Group heading="Clientes" className="text-[10px] font-semibold text-muted-foreground px-2 py-1.5 uppercase tracking-wider">
                {filteredClientes.map(c => (
                  <Command.Item key={c.id} value={c.nombre} onSelect={() => { setOpen(false); setView('clientes'); }}
                    className="flex items-center gap-3 px-3 py-2 rounded-lg text-sm cursor-pointer aria-selected:bg-accent aria-selected:text-accent-foreground">
                    <Users className="w-4 h-4 text-muted-foreground" />
                    <span>{c.nombre}</span>
                    <span className="ml-auto text-[10px] text-muted-foreground">{c.telefono || '—'}</span>
                  </Command.Item>
                ))}
              </Command.Group>
            )}
            ...
            {search.match(/^\d+/) && (
              <Command.Group heading="Presupuestos por monto" className="text-[10px] font-semibold text-muted-foreground px-2 py-1.5 uppercase tracking-wider">

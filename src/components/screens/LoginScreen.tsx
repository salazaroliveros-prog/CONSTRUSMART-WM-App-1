      ...
      <div className="relative z-10 w-full max-w-md">
        <div className="bg-white/95 dark:bg-slate-950/95 rounded-3xl shadow-2xl p-5 sm:p-6 md:p-8 border border-slate-200/70 dark:border-slate-700/60">
          <div className="text-center mb-4 sm:mb-6 animate-fade-in-up" style={{ animationDelay: '100ms' }}>
            ...
            <h1 className="text-lg sm:text-2xl font-bold text-slate-900 dark:text-white tracking-tight">CONSTRUCTORA WM/M&S</h1>
            <p className="text-xs sm:text-sm text-slate-500 dark:text-emerald-300 italic mt-1">Edificando el Futuro</p>
          ...
          <div className="flex gap-1 bg-slate-100/80 dark:bg-slate-900/70 p-1 rounded-2xl mb-4 sm:mb-5 animate-fade-in-up" style={{ animationDelay: '200ms' }}>
            <button
              ...
              className={`flex-1 py-2 text-xs font-semibold rounded transition ${mode === 'login' ? 'bg-white shadow text-slate-900 dark:bg-accent dark:text-blue-300' : 'text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white'}`}
            >Iniciar Sesión</button>
            <button
              type="button"
              onClick={() => { setMode('signup'); setInfo(null); }}
              className={`flex-1 py-2 text-xs font-semibold rounded transition ${mode === 'signup' ? 'bg-white shadow text-slate-900 dark:bg-accent dark:text-blue-300' : 'text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white'}`}
            >Crear Cuenta</button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-2 sm:space-y-3 animate-fade-in-up" style={{ animationDelay: '300ms' }}>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-3 py-2 sm:py-2.5 rounded-lg border border-slate-300/80 dark:border-slate-700 bg-slate-100/90 dark:bg-slate-900/80 text-slate-900 dark:text-white placeholder-slate-500 dark:placeholder-slate-400 focus:border-blue-400 focus:ring-2 focus:ring-blue-300/30 outline-none text-xs sm:text-sm"
            ...
            <div>
              <label className="text-xs font-semibold text-slate-700 dark:text-white mb-1 block">Correo electrónico</label>
              <div className="relative">
            ...
            <div>
              <label className="text-xs font-semibold text-slate-700 dark:text-white mb-1 block">Contraseña</label>
              <div className="relative">
            ...
            {authError && (
              <div className="bg-red-50 dark:bg-red-500/15 border border-red-200/80 dark:border-red-400/60 text-red-700 dark:text-red-100 text-xs p-2 sm:p-2.5 rounded-lg flex items-start gap-2">
                <AlertCircle className="w-4 h-4 shrink-0 mt-0.5" />
                <span>{authError}</span>
              </div>
            )}
            {info && (
              <div className="bg-emerald-50 dark:bg-emerald-500/15 border border-emerald-200/70 dark:border-emerald-400/60 text-emerald-900 dark:text-emerald-100 text-xs p-2 sm:p-2.5 rounded-lg">{info}</div>
            )}
